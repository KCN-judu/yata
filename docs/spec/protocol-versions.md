...
account than the profile's, `internal.import_mismatch` (tag 104) for an import
paired with other original bytes, and `internal.blob_failed` (tag 105) for a
blob the daemon could not encode. Version 1 stays reserved until commands and
jobs are in the file; until then a tag may still change.

...
| 固有属性               | `InnateAttribute`              | —                  | A boss soul's innate attribute; one of six `SoulAttribute`s, held apart from its sub-attributes. A boss soul carries it as `SoulKind::Boss` (ADR-0029). The import preview labels it 固定属性                |
...
| 评分档位               | `Tier`                         | —                  | N, R, SR, SSR, SP, UR: the quality tiers of `quality-model.md`, named after the game's rarity ladder but ordered by quality                                                                                  |
| 评分原型               | `Archetype`                    | —                  | Project-defined; a published quality profile of useful sub-attributes: 输出 `output`, 命中 `hit`, 治疗 `healing` (four each), and 速度 `speed` (Speed only, eligible at four roll units)                     |
| 套装效果               | `SetEffect`                    | Set Effect         | 2-piece and 4-piece variants                                                                                                                                                                                 |
| 需求画像               | `NeedProfile`                  | —                  | Project-defined; one Shikigami build's, role's, or set use's requirement, which pass 2 scores against ([scoring](scoring.md))                                                                                |
| 套装用途               | `SetUse`                       | —                  | Project-defined; a need profile for a four-piece set's usual use, read from its effect text; it only adds keep suggestions                                                                                   |
| 套装方案               | `SetPlan`                      | —                  | Project-defined; the set pieces a loadout needs, such as four of one set and two of any other                                                                                                                |
| 配装                   | `Loadout`                      | —                  | Project-defined; six slots of souls, one soul per slot, evaluated as a whole                                                                                                                                 |
| 式神基础属性           | `Baseline`                     | —                  | Project-defined; a Shikigami's own Speed, Crit, Crit DMG, Effect HIT and Effect RES, read or authored, for panel targets                                                                                     |
| 式神实例               | `ShikigamiInstanceId`          | —                  | Project-defined; the game's identifier of one owned copy of a Shikigami. `ShikigamiId` identifies the Shikigami itself                                                                                       |

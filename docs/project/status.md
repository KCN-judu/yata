...
| scheme code / SoulSelection (ADR-0009)    | partial     | layout codec (header, account, records); solved-bit edits; from-scratch sets; `scheme plans/retarget/build`                     | SoulSelection, the bit ↔ soul/attribute mapping in code, `matches`, the per-profile account fact, the encoding header (ISS-0007) | [evidence/testing.md](../evidence/testing.md)                        |
...
| scheme-code transport and QR (ADR-0009)   | tested      | Base64 ⇄ zlib ⇄ raw payload with limits; dump, diff, bit reading; QR matrix, PNG render and read; `yata-daemon scheme` commands | any payload field decoded; template editing                                                                                      | [evidence/testing.md](../evidence/testing.md)                        |

...
| #   | Item                                                                                                                                               | Gated on                                                                                    |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 2   | Community-format import (PRP-0008): the parser, the provenance of an imported file, and the fields `spec/import-format.md` marks unestablished     | files a user supplies                                                                       |
| 3   | The suit codes and the innate attribute, re-established from imported files (`scheme-code.md`, ADR-0014)                                           | 2                                                                                           |
| 4   | Import in the daemon: the imported file; the SQLite store through `yata-store` and the fact log (`fact-format.md`, ADR-0002, ADR-0019)             | 2                                                                                           |
...
| 8   | **Advanced filter**: score-based conditions over `quality.*` (`query.md`), layered on the official filter                                          | pass 1 in `yata-core` (`quality-model.md`)                                                  |
...
- pass 2 and matching (`scoring.md`, ADR-0028), and with them the advanced
  filter's `affinity(...)` conditions: a long-term plan, not this milestone's
